...
CREATE TABLE IF NOT EXISTS users (
    ...
    play_policy TEXT NOT NULL DEFAULT '3d' CHECK (
        play_policy IN ('3d', '2d', '1d')
    ),
...
CREATE TABLE IF NOT EXISTS booking_participants (
    ...
    CONSTRAINT user_or_email CHECK (
        user_id IS NOT NULL
...
CREATE INDEX IF NOT EXISTS idx_blocks_court ON blocks (
    court_id,
    start_date,
    end_date
);
...
CREATE INDEX IF NOT EXISTS idx_bookings_reminder ON bookings (
    status,
    start_time,
    reminder_sent
);

-- Prevent double booking same court and time
ALTER TABLE bookings
ADD CONSTRAINT unique_court_slot UNIQUE (
    court_id,
    start_time,
    end_time
);