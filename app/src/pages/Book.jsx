...
export default function Book() {
  ...
  // Load banners every minute
  useEffect(() => {
    const loadBanners = async () => {
      const now = new Date().toISOString();
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/bookings/cancellations?now=${now}`,
        { credentials: "include" }
      );
  ...
  // Adjust morning/evening labels dynamically
  ...
  function handleParticipantsChange(n) {
    ...
    setEmails(copy);
    if (count !== 2) setDuration("30");
  }

  // Load slots
  async function findSlots() {
    if (!date) return showToast("Select a date first", "warn");
    setLoading(true);
    setSearched(true);
    try {
      ...
      if ((data.slots || []).length === 0)
        showToast("No slots available for this selection", "warn");
      else showToast("Available slots loaded!", "success");
    } catch (err) {
  ...
  async function bookSlot() {
    if (!selected) return showToast("Select a slot first", "warn");
    for (const e of emails)
      if (!e.endsWith("nitw.ac.in"))
        return showToast(
          "All participant emails must end with nitw.ac.in",
          "warn"
        );
    ...
    try {
      ...
      showToast("✅ Booking created! Confirmation emails sent.", "success");
      setSlots([]);
      setSelected(null);
      setDate("");
  ...
  return (
    <div className="container">
      ...
      <div className="card grid booking-form">
        ...
        <div className="mt16">
          <div className="card">
            <p style={{ textAlign: "center", color: "var(--muted)" }}>
              No slots available for the selected date and time window. Try a
              different selection.
            </p>
          </div>
        </div>
